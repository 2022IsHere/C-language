...
bool parseInt(char *string, int *integer) {
	...
	while( index < length && !isspace(string[index]) ) {
		...
		integerBuffer[integerChars] = string[index];
		integerChars++;
		index++;
	}
	
	// Add this to end of the temporary string for further actions
	integerBuffer[integerChars] = '\0';
...
int totalStudents() {
	...
	do {
		...
		parsedCorrect = parseInt(buffer, &integer);
					
		// if issues in parsing integer
		if ( !parsedCorrect ) {
			printf("\n\nTraceback ( most recent call last):\n");
			printf("File: 'Student.c', line 206, in <module>\n");
